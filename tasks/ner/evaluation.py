...
class Evaluator:
    ...
    def evaluate_individual(self, individual):
        ...
        val_df = create_df(
            Path(self.base_dir) / "ner/data/dataset" / self.config["validation_file"]
        )
        val_df_sample = val_df.sample(
            n=int(len(val_df) * self.config["validation_sample_ratio"]), random_state=42
        )
...
def evaluate_llm_response(response_text, true_labels, tokens):
    ...
    for token in tokens:
        if token in entities and word_counts[token] < len(entities[token]):
            pred_labels.append(
                entities[token][word_counts[token]]
